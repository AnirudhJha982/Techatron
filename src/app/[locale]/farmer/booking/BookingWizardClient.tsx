...
export default function BookingWizardClient({ initialCentres = [] }: { initialCentres: any[] }) {
  ...
  const [centres, setCentres] = useState<any[]>(initialCentres)
  const [selectedCentre, setSelectedCentre] = useState<any>(initialCentres.length > 0 ? initialCentres[0] : null)
  ...
  useEffect(() => {
    // Only fetch offline if we don't have initial centres
  ...
  const filteredCentres = selectedStateFilter === 'ALL' 
    ? centres 
    : centres.filter(c => c.state === selectedStateFilter)
        ...
        <CardContent className="pt-6">
            ...
            <div className="space-y-4">
              ...
              {uniqueStates.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  <Button 
                    variant={selectedStateFilter === 'ALL' ? 'default' : 'outline'}
                    className={selectedStateFilter === 'ALL' ? 'bg-green-800 hover:bg-green-700 text-white' : 'text-slate-600 hover:bg-slate-100'}
                  ...
                  {uniqueStates.map(state => (
                    <Button
                      key={state}
                      variant={selectedStateFilter === state ? 'default' : 'outline'}
                      className={selectedStateFilter === state ? 'bg-green-800 hover:bg-green-700 text-white' : 'text-slate-600 hover:bg-slate-100'}
                      onClick={() => setSelectedStateFilter(state)}
                      size="sm"
                    >
                      {translateState(state, locale)}
                    </Button>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCentres.map((c) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 mt-1">District: {c.district}</p>
                    <p className="text-xs text-slate-500 mt-1">{c.address}</p>
                    <p className="text-xs font-semibold text-slate-700 mt-2">Operating Hours: 8:00 AM - 5:00 PM</p>
                    <p className="text-xs font-semibold text-green-800 mt-2">Capacity: {c.capacityPerDay} Qtl / Day</p>
                  </div>
                ))}
              </div>
          ...
          {step === 5 && bookingResult && (
            <div className="text-center py-8 space-y-6">
              <div className={`w-20 h-20 ${bookingResult.isOffline ? 'bg-amber-100' : 'bg-green-100'} rounded-full flex items-center justify-center mx-auto text-4xl shadow-sm`}>
                {bookingResult.isOffline ? '⏳' : '✅'}
              </div>
              
              {bookingResult.isOffline ? (
                <>
                  <h2 className="text-3xl font-black text-amber-950">Booking saved offline</h2>
                  <p className="text-slate-600 text-sm max-w-md mx-auto font-bold text-amber-700">Booking request saved and waiting for synchronization.</p>
                  <p className="text-slate-500 text-xs max-w-md mx-auto">Your booking will be finalized when internet connection is restored.</p>
                </>
              ) : (
                <>
              ...
              {/* Digital Token Card */}
              <div className={`bg-gradient-to-br ${bookingResult.isOffline ? 'from-amber-900 via-amber-800 to-amber-950 border-amber-300' : 'from-green-900 via-green-850 to-green-950 border-yellow-400'} text-white p-8 rounded-2xl border-4 max-w-md mx-auto shadow-xl text-left relative overflow-hidden`}>
                <div className="flex justify-between items-center mb-4">
                  <span className="bg-yellow-400 text-green-950 text-[10px] font-black px-2.5 py-0.5 rounded uppercase">{bookingResult.isOffline ? 'Offline Pass' : 'Govt Digital Pass'}</span>
